...
def get_opening_times(station_id):
    ...
    try:
        ...
        return r["station"]["openingTimes"] if r.get("ok") else None
    except: return None
        ...
        with tab:
            ...
            for i, s in enumerate(sorted_list):
                ...
                <div style="background:{bg}; padding:15px; border-radius:12px; margin-top:10px; border-left:8px solid {border}; display:flex; justify-content:space-between; align-items:center; box-shadow:2px 2px 5px rgba(0,0,0,0.1);">
                    <div><b>{"⭐ " if is_decker else ""}{brand_name}</b><br><small>{s["place"]}, {s["street"]}</small></div>
                    <div style="text-align:right;"><div style="font-weight:bold; font-size:1.3rem;">{s[fuel_type]:.2f} €</div></div>
                </div>
                ''', unsafe_allow_html=True)
                
                # DETAILS & ÖFFNUNGSZEITEN
                with st.expander(f"🕒 Öffnungszeiten für {brand_name}"):
                    status = "🟢 JETZT GEÖFFNET" if s["isOpen"] else "🔴 GESCHLOSSEN"
                    st.write(f"Status: **{status}**")
                    
                    # Hier ist der Button mit besserem Text
                    if st.button(f"Wochenplan anzeigen", key=f"btn_{s['id']}"):
                        times = get_opening_times(s['id'])
                        if times:
                            for entry in times:
                                st.write(f"**{entry['text']}**: {entry['start']} - {entry['end']} Uhr")
                        else:
                            st.warning("Diese Tankstelle hat leider keine detaillierten Wochenzeiten gemeldet.")
