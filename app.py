    ...
    def render_fuel(tab, fuel_type):
        with tab:
            ...
            for i, s in enumerate(sorted_list):
                ...
                # Ausklappbare Infos (Öffnungszeiten & Status)
                status_text = "🟢 JETZT GEÖFFNET" if s["isOpen"] else "🔴 AKTUELL GESCHLOSSEN"
                with st.expander(f"Details & Öffnungszeiten für {brand_name}"):
                    st.write(f"**Status:** {status_text}")
                    st.write("**Adresse:**")
                    st.code(f"{s['street']}\n{s['postCode']} {s['place']}")
                    st.info("Die genauen täglichen Öffnungszeiten werden direkt von der Tankstelle an die Markttransparenzstelle gemeldet.")

    render_fuel(t1, "e5")
    render_fuel(t2, "e10")
    render_fuel(t3, "diesel")
else:
    st.warning("Preise werden geladen... bitte kurz warten.")

# 3. SERVICE BEREICH
st.write("---")
st.subheader("🛠️ Wiesmoor Direkt-Hilfe")
if st.button("📞 Gebr. Decker direkt anrufen"):
    st.success("Telefon: 04948 91990")
